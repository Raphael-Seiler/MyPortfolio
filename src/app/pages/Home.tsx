...
import { Zap, Target, Coffee, Car, Fish, Users } from 'lucide-react';
      ...
      <section className="relative min-h-[70vh] flex flex-col md:flex-row items-center justify-between pt-10 pb-20 overflow-hidden gap-16">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-semibold tracking-tighter text-[#1d1d1f] dark:text-[#f5f5f7] mb-8 leading-[1.05] drop-shadow-sm dark:drop-shadow-md">
            Hallo, ich bin<br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#1d1d1f] to-[#55555a] dark:from-[#f5f5f7] dark:to-[#d1d1d6]">Raphi.</span>
          </h1>
          <p className="text-xl md:text-2xl text-[#55555a] dark:text-[#e5e5ea] max-w-xl leading-relaxed font-light">
            Ein angehender UX-Designer mit dem Ziel, komplexe Probleme durch empathisches und minimalistisches Design zu lösen.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[minmax(200px,auto)] relative z-10">
          ...
          >
            <h2 className="text-2xl font-semibold mb-6 text-[#1d1d1f] dark:text-[#f5f5f7] flex items-center gap-3">
              <span className="w-6 h-6 text-[#55555a] dark:text-[#e5e5ea]">💡</span>
              Meine Philosophie
            </h2>
            <p className="text-[#55555a] dark:text-[#e5e5ea] leading-relaxed font-light text-lg">
              Meine Arbeit soll etwas Positives in der Welt bewegen. Mich fasziniert es, komplizierte Probleme anzupacken und einfache, gute Lösungen dafür zu finden. Dabei ist es mir besonders wichtig, dass meine Designs für alle Menschen leicht verständlich und ohne Hürden nutzbar sind. Denn gutes Design sollte niemanden ausschliessen, sondern den Alltag für jeden ein bisschen besser machen.
            </p>
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { icon: <Car className="w-6 h-6 mb-2 text-[#1d1d1f] dark:text-[#f5f5f7]" />, label: "Auto-Fanatiker" },
                { icon: <Fish className="w-6 h-6 mb-2 text-[#1d1d1f] dark:text-[#f5f5f7]" />, label: "Fischer" },
                { icon: <Coffee className="w-6 h-6 mb-2 text-[#1d1d1f] dark:text-[#f5f5f7]" />, label: "Kaffee-Geniesser" },
                { icon: <Users className="w-6 h-6 mb-2 text-[#1d1d1f] dark:text-[#f5f5f7]" />, label: "Familie & Freunde" }
              ].map((hobby, i) => (
                <div key={i} className="bg-transparent border border-black/5 dark:border-white/10 rounded-2xl p-4 flex flex-col items-center justify-center text-center hover:border-black/20 dark:hover:border-white/30 transition-colors">
                  {hobby.icon}
                  <span className="text-sm font-medium text-[#55555a] dark:text-[#e5e5ea]">{hobby.label}</span>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <section className="pt-32 pb-10 relative z-10">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-[#55555a] dark:text-[#e5e5ea] font-light">Ein visueller Vorgeschmack auf meine Arbeiten.</p>
          </div>
          <Link 
            to="/projects" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[250px]">
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-8">
              <span className="text-white font-medium text-lg">Fintech Mobile App</span>
            </div>
          </motion.div>
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
              <span className="text-white font-medium text-sm">Wireframes & Sketches</span>
            </div>
          </motion.div>
